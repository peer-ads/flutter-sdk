...
// ── Shared rewarded-ad HTML template (Dart) ───────────────────────────────────
//
...
String buildRewardedAdHtml({
  ...
  // Escape </script> to prevent premature tag close
  final data   = jsonEncode(cfg).replaceAll('</', r'<\/');
  const bridge = "window.__paBridge=function(e,d){PeerAdsChannel.postMessage(JSON.stringify({event:e,data:d}));};";
  final inject = 'window.__PA=$data;$bridge';

  return '<!DOCTYPE html><html lang="en"><head>'
      '<meta charset="UTF-8">'
      '<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">'
      '<style>$_kCss</style></head>'
      '<body><script>$inject</script>'
      '$_kBody'
      '<script>$_kJs</script></body></html>';
}
